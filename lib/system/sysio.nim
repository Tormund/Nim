...
when defined(windows):
  proc seeki(f: cint, offset: int64, whence: cint): int64={.
    importc: "_lseeki64", header: "<io.h>", tags: [].}
  proc telli(f: cint): int64 {.
    importc: "_telli64", header: "<io.h>", tags: [].}
  proc c_fseek(f: File, offset: int64, whence: cint): cint = seeki(c_fileno(f), offset, whence)
  proc c_ftell(f: File): int64 = c_telli(c_fileno(f))
else:
